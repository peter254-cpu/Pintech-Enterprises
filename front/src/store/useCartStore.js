...
export const useCartStore = create((set, get) => ({
    ...
    removeFromCart: async (productId) => {
      ...
      clearCart: async()  => {
        try {
            // Update the local state to clear the cart 
            set({ cart: [] }); 
            // Recalculate the totals after clearing the cart 
            get().calculateTotals();
        } catch (error) {
            // Handle errors by showing an error message 
            toast.error(error.response?.data?.message || "Error occurred while clearing the cart"); 
            console.log(error);
        }
